    ...
    <section className="relative py-24 md:py-32 overflow-hidden">
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="grid gap-5 sm:grid-cols-2 max-w-5xl mx-auto">
          {keys.map((key, i) => (
            <motion.div key={key} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.7, delay: i * 0.12 }} whileHover={{ scale: 1.03, transition: { duration: 0.3 } }}
              className="group relative glass card-shine rounded-3xl p-8 md:p-10 overflow-hidden transition-all duration-500 hover:bg-white/[0.04] hover:border-amber-500/30 hover:shadow-[0_0_30px_-5px_rgba(201,169,97,0.15)]">
              <span className="absolute -top-4 -end-2 text-[8rem] md:text-[10rem] font-black leading-none pointer-events-none select-none" style={{ background: "linear-gradient(180deg, rgba(201,169,97,0.12) 0%, transparent 80%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>{nums[i]}</span>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.7, delay: 0.5 }} className="mt-16 text-center">
          <a href="https://crm.wtradersworld.com" target="_blank" rel="noopener noreferrer" className="group inline-flex items-center gap-2 rounded-full border border-amber-500/60 bg-amber-500/10 px-7 py-3 text-sm font-semibold text-white transition-all duration-300 hover:bg-amber-500/20 hover:border-amber-500">
            {t("fourReasons.cta")}
